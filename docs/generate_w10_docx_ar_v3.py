...
COLOR_PRIMARY_DARK = RGBColor(0x1E, 0x1B, 0x4B)   # كحلي إمبراطوري غامق للعناوين
COLOR_BRAND = RGBColor(0x43, 0x38, 0xCA)          # نيلي ملكي للعناوين الفرعية
COLOR_ACCENT = RGBColor(0xC9, 0x97, 0x1F)         # ذهبي فاخر للإبراز
COLOR_TEXT = RGBColor(0x11, 0x18, 0x27)           # أسود داكن عريض وواضح للقراءة
COLOR_MUTED = RGBColor(0x4B, 0x55, 0x63)          # رمادي معتدل للنصوص الوصفية
COLOR_GREEN = RGBColor(0x06, 0x5F, 0x46)          # أخضر رسمي لنتائج الفحص
...
def set_table_styling(table, border_color="D1D5DB", border_sz="4"):
...
def add_arabic_heading(doc, text, level=1):
    ...
    if level == 1:
        ...
        pBdr = parse_xml(
            f'<w:pBdr {nsdecls("w")}>'
            f'<w:bottom w:val="single" w:sz="16" w:space="6" w:color="4338CA"/>'
            f'</w:pBdr>'
        )
...
def add_card_box(doc, title, content_list, border_color="4338CA", bg_color="EEF2FF"):
    ...
    borders = parse_xml(
        ...
        f'<w:left w:val="none"/>'
        f'<w:right w:val="single" w:sz="28" w:space="0" w:color="{border_color}"/>'
...
def add_code_block(doc, code_text):
    ...
    borders = parse_xml(
        ...
        f'<w:left w:val="single" w:sz="16" w:space="0" w:color="1E1B4B"/>'
...
def add_figure_with_caption(doc, image_path, caption_arabic, width_inches=6.0, is_vertical=False):
    ...
    img_w = width_inches - 0.4 if not is_vertical else 3.2
    ...
    set_run_font(r_cap, font_name=FONT_ARABIC, size_pt=12, bold=True, color=COLOR_PRIMARY_DARK)

    doc.add_paragraph().paragraph_format.space_after = Pt(8)

def build_complete_illustrated_document():
    doc = docx.Document()
    ...
    set_run_font(r_inst, font_name=FONT_ARABIC, size_pt=14.5, bold=True, color=RGBColor(0x37, 0x41, 0x51))

    p_rule = doc.add_paragraph()
    p_rule.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r_rule = p_rule.add_run("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    set_run_font(r_rule, font_name=FONT_ENGLISH, size_pt=11, bold=True, color=COLOR_BRAND)
    p_rule.paragraph_format.space_after = Pt(16)

    p_title = doc.add_paragraph()
    set_paragraph_rtl(p_title, WD_ALIGN_PARAGRAPH.CENTER)
    p_title.paragraph_format.space_after = Pt(12)
    r_t1 = p_title.add_run("التقرير الفني النهائي لتوثيق المشروع البرمجي الشامل\n")
    set_run_font(r_t1, font_name=FONT_ARABIC, size_pt=24, bold=True, color=COLOR_PRIMARY_DARK)

    r_t2 = p_title.add_run("نظام حجز وإدارة الفنادق المتكامل (Enterprise Hotel Booking System)\n")
    set_run_font(r_t2, font_name=FONT_ARABIC, size_pt=17.5, bold=True, color=COLOR_BRAND)

    r_t3 = p_title.add_run("دليل التدريب الميداني — محاكاة متكاملة لدورة حياة تطوير البرمجيات (SDLC 2026 - 2027م)\n")
    set_run_font(r_t3, font_name=FONT_ARABIC, size_pt=13.5, bold=False, color=COLOR_MUTED)

    p_rule2 = doc.add_paragraph()
    p_rule2.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r_rule2 = p_rule2.add_run("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    set_run_font(r_rule2, font_name=FONT_ENGLISH, size_pt=11, bold=True, color=COLOR_BRAND)
    p_rule2.paragraph_format.space_after = Pt(20)

    # جدول بيانات الغلاف
    meta_table = doc.add_table(rows=8, cols=2)
    meta_table.alignment = WD_TABLE_ALIGNMENT.CENTER
    set_table_styling(meta_table, border_color="E5E7EB", border_sz="6")

    meta_items = [
        ("عنوان المشروع البرمجي", "نظام إدارة وحجز الفنادق الموزع متعدد الطبقات (Hotel Booking System)"),
        ("المساق الأكاديمي والرمز", "دليل التدريب الميداني لمشاريع تطوير البرمجيات الفردية (IT-492)"),
        ("المشرف الأكاديمي على المساق", "أ.د / إبراهيم أحمد البلطة (أستاذ هندسة البرمجيات المشارك)"),
        ("إعداد مهندس البرمجيات / الطالب", "لؤي السروري (Loai Alsorory)"),
        ("المعمارية البرمجية المعتمدة", "Clean Architecture + Repository Pattern + Provider (Flutter)"),
        ...
        ("تاريخ التسليم والإصدار", "الإصدار النهائي الشامل 1.0.0 — سبتمبر 2026م")
    ]
    ...
    for idx, (label, val) in enumerate(meta_items):
        ...
        set_paragraph_rtl(p0, WD_ALIGN_PARAGRAPH.RIGHT)
        r0 = p0.add_run(label)
        set_run_font(r0, font_name=FONT_ARABIC, size_pt=12.5, bold=True, color=COLOR_PRIMARY_DARK)

        p1 = cell_val.paragraphs[0]
        set_paragraph_rtl(p1, WD_ALIGN_PARAGRAPH.RIGHT)
        r1 = p1.add_run(val)
        set_run_font(r1, font_name=FONT_ARABIC, size_pt=12, bold=False, color=COLOR_TEXT)
    ...
    set_table_styling(table_arch, border_color="E5E7EB", border_sz="4")
    ...
    set_table_styling(table_req, border_color="E5E7EB", border_sz="4")
    ...
    add_figure_with_caption(
        ...
        "الشكل (1): واجهة توثيق واختبار نقاط النهاية البرمجية (RESTful APIs) عبر Swagger UI"
    )
    ...
    add_arabic_heading(doc, "1.3 دليل استخدام بوابة الويب الإدارية مدعماً بالصور (ASP.NET Core MVC Portal):", level=2)

    # 1. شاشة تسجيل الدخول
    add_card_box(
        doc,
        "1. واجهة تسجيل الدخول المنقسمة (Split-Screen Login)",
        [
            "تعتمد تصميماً عصرياً؛ يبرز الجانب الأيمن واجهة ترحيبية بصور الفنادق الفاخرة، بينما يقدم الجانب الأيسر نموذج تسجيل دخول ذكي مع التحقق الفوري من صحة البريد الإلكتروني وكلمة المرور وإصدار كوكيز مصادقة آمنة ومشفرة."
        ],
        border_color="4338CA",
        bg_color="F8FAFC"
    )
    add_figure_with_caption(
        doc,
        os.path.join(sc_dir, "fig01_web_login.png"),
        "الشكل (2): واجهة تسجيل الدخول المنقسمة بنظام المصادقة الآمن (Split-Screen Login)"
    )

    # 2. لوحة التحكم
    add_card_box(
        doc,
        "2. لوحة التحكم المركزية (Main Dashboard)",
        [
            "تستعرض 4 مؤشرات أداء رئيسية (KPIs) بحسابات رقمية حية: إجمالي الفنادق المسجلة، عدد الغرف الشاغرة، إجمالي الحجوزات النشطة، وصافي الإيرادات المحصلة. كما تحتوي على جدول الحجوزات الأخيرة وأزرار وصول سريع لإضافة فندق أو غرفة فوراً."
        ],
        border_color="4338CA",
        bg_color="F8FAFC"
    )
    add_figure_with_caption(
        ...
        "الشكل (3): لوحة التحكم المركزية وبطاقات مؤشرات الأداء الحية وجدول الحجوزات (Main Dashboard)"
    )

    # 3. إدارة الفنادق
    add_card_box(
        doc,
        "3. شاشة إدارة الفنادق (Hotels Management)",
        [
            "استعراض بيانات الفنادق بجدول تفاعلي مدعوم بـ DataTable.js يتيح البحث اللحظي بالاسم والمدينة وتصدير البيانات. تتيح شاشة الإنشاء إضافة الفندق بصورته وتقييمه، وشاشة التفاصيل تستعرض بطاقة الفندق وقائمة غرفه."
        ],
        border_color="4338CA",
        bg_color="F8FAFC"
    )
    add_figure_with_caption(
        doc,
        os.path.join(sc_dir, "fig03_web_hotels.png"),
        "الشكل (4): شاشة إدارة الفنادق المجهزة بجدول تفاعلي يدعم البحث والفرز اللحظي (Hotels Management)"
    )

    # 4. إدارة الغرف
    add_card_box(
        doc,
        "4. شاشة إدارة الغرف الفندقية وحالة التوفر (Rooms Inventory)",
        [
            "تمكين المشرف من إضافة وتعديل الغرف وربطها بالفندق، مع تحديد نوع الغرفة (مفردة، مزدوجة، جناح، ديلوكس)، وتحديد طاقتها الاستيعابية، وسعر الليلة، وتعديل حالة الغرفة بين (متاحة / محجوزة) بضغطة زر."
        ],
        border_color="4338CA",
        bg_color="F8FAFC"
    )
    add_figure_with_caption(
        ...
        "الشكل (5): شاشة إدارة الغرف الفندقية وتحديد الأسعار وحالات التوفر (Rooms Inventory)"
    )

    # 5. إدارة الحجوزات
    add_card_box(
        doc,
        "5. شاشة إدارة ومتابعة دورة حياة الحجوزات (Bookings Workflow)",
        [
            "إمكانية إنشاء حجز فندقي جديد مع احتساب السعر الإجمالي تلقائياً بضرب عدد الليالي في سعر الغرفة، مع تفعيل محرك منع التضارب الزمني، وإمكانية تعديل حالة الحجز بين (مؤكد Confirmed، معلق Pending، وملغي Cancelled)."
        ],
        border_color="4338CA",
        bg_color="F8FAFC"
    )
    add_figure_with_caption(
        ...
        "الشكل (6): شاشة إدارة ومتابعة دورة حياة الحجوزات والعمليات المالية (Bookings Management)"
    )

    # 6. نافذة تأكيد الحذف المنبثقة
    add_card_box(
        doc,
        "6. نافذة تأكيد الحذف المنبثقة (Bootstrap Modal Confirmation)",
        [
            "تطبيق متطلب الـ Modal التفاعلي لمنع الحذف العرضي أو غير المقصود للبيانات، حيث تظهر نافذة تأكيد أنيقة باللون الأحمر للتحقق من رغبة المشرف قبل إتمام عملية الحذف من قاعدة البيانات."
        ],
        border_color="C9971F",
        bg_color="FFFBEB"
    )
    add_figure_with_caption(
        doc,
        os.path.join(sc_dir, "fig06_web_delete_modal.png"),
        "الشكل (7): نافذة تأكيد الحذف المنبثقة التفاعلية لمنع حذف السجلات بالخطأ (Bootstrap Modal)"
    )

    doc.add_page_break()

    # -------------------------------------------------------------
    # دليل تطبيق الموبايل مدعماً بالصور
    # -------------------------------------------------------------
    add_arabic_heading(doc, "2.3 دليل استخدام تطبيق الهاتف المحمول مدعماً بالصور (Flutter Mobile Application):", level=2)

    # 7. الشاشة الرئيسية للموبايل
    add_card_box(
        doc,
        "1. لوحة تحكم تطبيق الهاتف المحمول (HomeScreen Dashboard)",
        [
            "تستعرض بطاقات إحصائية للمستخدم، شريط ترحيب باسم العميل، شريط أفقي فخم لأبرز الفنادق الموصى بها بصورها المحملة بسلاسة، وأزرار التنقل السريع."
        ],
        border_color="7C3AED",
        bg_color="FAF5FF"
    )
    add_figure_with_caption(
        ...
        "الشكل (8): لوحة تحكم تطبيق الهاتف المحمول وعرض الفنادق الموصى بها (HomeScreen Dashboard)"
    )

    # 8. القائمة الجانبية المنسحبة
    add_card_box(
        doc,
        "2. القائمة الجانبية المنسحبة (RTL Navigation Drawer)",
        [
            "قائمة سحب ناعمة تنزلق من اليمين (RTL) تضم ترويسة أنيقة باسم المستخدم وبريده وصلاحيته، وروابط الانتقال لجميع أقسام التطبيق، مع زر تسجيل الخروج الآمن."
        ],
        border_color="7C3AED",
        bg_color="FAF5FF"
    )
    add_figure_with_caption(
        doc,
        os.path.join(sc_dir, "fig09_mobile_drawer.png"),
        "الشكل (9): القائمة الجانبية المنسحبة من اليمين الداعمة للهوية العربية (RTL Navigation Drawer)"
    )

    # 9. كتالوج الفنادق في الموبايل
    add_card_box(
        doc,
        "3. كتالوج تصفح الفنادق بالبحث الفوري (Hotels Catalog)",
        [
            "شاشة استعراض الفنادق مع شريط بحث بالاسم والمدينة، وبطاقات فوتوغرافية فاخرة تعكس تصنيف النجوم وموقع الفندق."
        ],
        border_color="7C3AED",
        bg_color="FAF5FF"
    )
    add_figure_with_caption(
        doc,
        os.path.join(sc_dir, "fig10_mobile_hotels.png"),
        "الشكل (10): كتالوج تصفح الفنادق بالبحث اللحظي في تطبيق الهاتف (Hotels Catalog)"
    )

    # 10. تفاصيل الفندق والغرف
    add_card_box(
        doc,
        "4. شاشة تفاصيل الفندق والغرف المتاحة (Hotel Details & Rooms)",
        [
            "شاشة تفاصيل الفندق الفاخرة التي تستعرض المرافق وقائمة الغرف المتاحة التابعة له، مع بطاقات الأسعار وزر الحجز المباشر."
        ],
        border_color="7C3AED",
        bg_color="FAF5FF"
    )
    add_figure_with_caption(
        doc,
        os.path.join(sc_dir, "fig11_mobile_hotel_details.png"),
        "الشكل (11): شاشة تفاصيل الفندق والمرافق واستعراض الغرف التابعة له (Hotel Details & Rooms)"
    )

    # 11. النافذة المنبثقة السفلية SheetModal
    add_card_box(
        doc,
        "5. النافذة السفلية المنبثقة لحجز الغرفة (Interactive SheetModal)",
        [
            "عند الضغط على زر 'حجز الآن' بأي غرفة، تنبثق نافذة سفلية تفاعلية تمكن النزيل من اختيار تاريخي الوصول والمغادرة وعدد الضيوف ومراجعة السعر الإجمالي وتأكيد الحجز مع ظهور إشعار نجاح عائم (SnackBar)."
        ],
        border_color="7C3AED",
        bg_color="FAF5FF"
    )
    add_figure_with_caption(
        ...
        "الشكل (12): النافذة السفلية المنبثقة لحجز الغرفة واحتساب الإجمالي تلقائياً (Interactive SheetModal)"
    )

    # 12. سجل الحجوزات في الهاتف
    add_card_box(
        doc,
        "6. شاشة سجل الحجوزات ومتابعة الحالات (Mobile Bookings Screen)",
        [
            "استعراض كافة الحجوزات السابقة والحالية للنزيل مع تواريخ الدخول والخروج ورقم الغرفة والقيمة الإجمالية وشارات الحالة الملونة."
        ],
        border_color="7C3AED",
        bg_color="FAF5FF"
    )
    add_figure_with_caption(
        doc,
        os.path.join(sc_dir, "fig13_mobile_bookings.png"),
        "الشكل (13): شاشة استعراض سجل الحجوزات ومتابعة الحالات في الهاتف (Bookings Screen)"
    )

    # 13. التصميم المتجاوب بنمط شاشة الهاتف
    add_card_box(
        doc,
        "7. التصميم المتجاوب بنمط شاشة الهاتف المحمول (Responsive Phone Frame)",
        [
            "حل هندسي متقدم يضمن ظهور التطبيق بأبعاد شاشة الهاتف المحمول المتناسقة دون حدوث أي أخطاء تجاوز بكسلات (Pixel Overflow) على مختلف مقاسات الشاشات."
        ],
        border_color="4338CA",
        bg_color="EEF2FF"
    )
    add_figure_with_caption(
        doc,
        os.path.join(sc_dir, "fig14_mobile_phone_frame.png"),
        "الشكل (14): التصميم المتجاوب بنمط شاشة الهاتف المحمول وحل مشكلة البكسلات (Responsive Phone Frame)"
    )
    ...
    add_arabic_heading(doc, "1.4 نتائج اختبارات واجهات الـ RESTful Web API عبر Postman و Swagger:", level=2)

    table_tests = doc.add_table(rows=11, cols=5)
    table_tests.alignment = WD_TABLE_ALIGNMENT.CENTER
    set_table_styling(table_tests, border_color="E5E7EB", border_sz="4")
    ...
    for row_idx, data in enumerate(tests_data, start=1):
        ...
        for col_idx, val in enumerate(data):
            ...
            r = p.add_run(val)
            is_result = (col_idx == 4)
            set_run_font(r, font_name=FONT_ARABIC, size_pt=11.5, bold=is_result, color=COLOR_GREEN if is_result else COLOR_TEXT)
    ...
    set_table_styling(table_val, border_color="E5E7EB", border_sz="4")
    ...
    for row_idx, data in enumerate(val_data, start=1):
        ...
        for col_idx, val in enumerate(data):
            ...
            set_paragraph_rtl(p, WD_ALIGN_PARAGRAPH.RIGHT)
            r = p.add_run(val)
            set_run_font(r, font_name=FONT_ARABIC, size_pt=11.5, bold=(col_idx == 0), color=COLOR_TEXT)
    ...
    set_table_styling(table_weeks, border_color="E5E7EB", border_sz="4")
    ...
    for row_idx, data in enumerate(weeks_summary, start=1):
        ...
        for col_idx, val in enumerate(data):
            ...
            is_w = (col_idx == 0)
            is_weight = (col_idx == 2)
            set_run_font(r, font_name=FONT_ARABIC, size_pt=12, bold=(is_w or is_weight), color=COLOR_BRAND if is_weight else COLOR_TEXT)
    ...
    for idx, ref in enumerate(refs_ar, start=1):
        ...
        set_run_font(r, font_name=FONT_ARABIC, size_pt=12.5, bold=False, color=COLOR_TEXT)
    ...
    print("Saved illustrated report to:", output_path_final)
...
if __name__ == "__main__":
    build_complete_illustrated_document()
